...
def test_datetime_microsecond_min():
    ...
    """
    assert lise_ormsgpack.packb(datetime.datetime(2000, 1, 1, 0, 0, 0, 1)) == msgpack.packb(
...
def test_datetime_omit_microseconds_naive():
    ...
    assert lise_ormsgpack.packb(
        [datetime.datetime(2000, 1, 1, 2, 3, 4, 123)],
        option=lise_ormsgpack.OPT_NAIVE_UTC | lise_ormsgpack.OPT_OMIT_MICROSECONDS,
...
def test_time_omit_microseconds():
    ...
    """
    assert lise_ormsgpack.packb(
        [datetime.time(2, 3, 4, 123)], option=lise_ormsgpack.OPT_OMIT_MICROSECONDS
...
def test_datetime_utc_z_naive():
    ...
    assert lise_ormsgpack.packb(
        [datetime.datetime(2000, 1, 1, 2, 3, 4, 123)],
        option=lise_ormsgpack.OPT_NAIVE_UTC | lise_ormsgpack.OPT_UTC_Z,
...
def test_datetime_utc_z_without_tz():
    ...
    """
    assert lise_ormsgpack.packb(
        [datetime.datetime(2000, 1, 1, 2, 3, 4, 123)], option=lise_ormsgpack.OPT_UTC_Z
    ) == msgpack.packb(["2000-01-01T02:03:04.000123"])
...
def test_datetime_roundtrip():
    ...
    deserialized = lise_ormsgpack.unpackb(lise_ormsgpack.packb(obj))
...
def test_date():
    ...
    datetime.date
    """
    assert lise_ormsgpack.packb([datetime.date(2000, 1, 13)]) == msgpack.packb(
        ["2000-01-13"]
    )
...
def test_date_min():
    ...
    """
    assert lise_ormsgpack.packb([datetime.date(datetime.MINYEAR, 1, 1)]) == msgpack.packb(
        ["0001-01-01"]
    )
...
def test_date_max():
    ...
    """
    assert lise_ormsgpack.packb([datetime.date(datetime.MAXYEAR, 12, 31)]) == msgpack.packb(
        ["9999-12-31"]
    )
...
def test_time():
    ...
    datetime.time
    """
    assert lise_ormsgpack.packb([datetime.time(12, 15, 59, 111)]) == msgpack.packb(
        ["12:15:59.000111"]
    )
    assert lise_ormsgpack.packb([datetime.time(12, 15, 59)]) == msgpack.packb(["12:15:59"])
...
def test_time_microsecond_max():
    ...
    """
    assert lise_ormsgpack.packb(datetime.time(0, 0, 0, 999999)) == msgpack.packb(
        "00:00:00.999999"
    )
...
def test_time_microsecond_min():
    ...
    """
    assert lise_ormsgpack.packb(datetime.time(0, 0, 0, 1)) == msgpack.packb(
        "00:00:00.000001"
    )
...
def test_passthrough_datetime():
    with pytest.raises(lise_ormsgpack.MsgpackEncodeError):
        lise_ormsgpack.packb(
            datetime.datetime(1970, 1, 1), option=lise_ormsgpack.OPT_PASSTHROUGH_DATETIME
        )
...
def test_passthrough_date():
    with pytest.raises(lise_ormsgpack.MsgpackEncodeError):
        lise_ormsgpack.packb(
            datetime.date(1970, 1, 1), option=lise_ormsgpack.OPT_PASSTHROUGH_DATETIME
        )
...
def test_passthrough_time():
    with pytest.raises(lise_ormsgpack.MsgpackEncodeError):
        lise_ormsgpack.packb(
            datetime.time(12, 0, 0), option=lise_ormsgpack.OPT_PASSTHROUGH_DATETIME
        )
